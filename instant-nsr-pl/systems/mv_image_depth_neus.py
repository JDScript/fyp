...
from datasets.mv_image_depth import MVImageDepthIterableDataset, MVImageDepthDataset
...
@systems.register("mv-image-depth-neus-system")
class MVImageDepthNeuSSystem(BaseSystem):
    """
    ...
    def preprocess_data(self, batch, stage):
        assert isinstance(self.dataset, MVImageDepthDataset) or isinstance(
            self.dataset, MVImageDepthIterableDataset
        )
        ...
        if stage in ["train"]:
            ...
            depth = (
                self.dataset.all_depths[index, y, x]
                .view(-1, self.dataset.all_depths.shape[-1])
                .to(self.rank)
            )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss_depth = ranking_loss(
            ...
            penalize_ratio=self.config.system.loss.depth_p_ratio,
            type="mean",
        )
        self.log("train/loss_depth", loss_depth, prog_bar=True, rank_zero_only=True)
