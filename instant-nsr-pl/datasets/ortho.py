...
def load_a_prediction(
    ...
    RT_front = np.loadtxt(
        os.path.join(cam_pose_dir, "_front_RT.txt")
    )  # world2cam matrix
    ...
    for idx, view in enumerate(view_types):
        ...
        normal_filepath = os.path.join(root_dir, test_object, "normal_%s.png" % (view))
        # Load key frame
        if load_color:  # use bgr
            image = np.array(
                PIL.Image.open(normal_filepath.replace("normal", "rgb")).resize(imSize)
            )[:, :, :3]
        ...
        color_mask = np.array(
            PIL.Image.open(
                os.path.join(root_dir, test_object, "rgb_%s.png" % (view))
            ).resize(imSize)
        )[:, :, 3]
        ...
        RT = np.loadtxt(
            os.path.join(cam_pose_dir, "_%s_RT.txt" % (view))
        )  # world2cam matrix
