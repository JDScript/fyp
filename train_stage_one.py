    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet):
                ...
                camera_embeddings = batch["camera_embeddings"]  # (B, Nv, Nce)

                # concatenate task embedding to camera embeddings
                task_embeddings = batch["task_embeddings"]
                ...
                # (B*Nv, Nce)
                camera_embeddings = rearrange(
                    camera_embeddings, "B Nv Nce -> (B Nv) Nce"
                )
                # camera embbeddings e_de_da_sincos
                camera_embeddings = torch.cat(
                    [torch.sin(camera_embeddings), torch.cos(camera_embeddings)], dim=-1
                )

                imgs_in, imgs_out, camera_embeddings = (
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="./configs/training/mv_image.yaml",
    )
