...
from models.blocks.cross_attention import (
        ...
        if from_flax:
            ...
            for key in state_dict_v0:
                ...
                if "norm_joint." in key:
                    tmp = copy.deepcopy(key)
                    state_dict[key.replace("norm_joint.", "norm_joint_last.")] = (
                        state_dict.pop(tmp)
                    )
                if "attn_joint_twice." in key:
                    tmp = copy.deepcopy(key)
                    state_dict[key.replace("attn_joint_twice.", "attn_joint_mid.")] = (
                        state_dict.pop(tmp)
                    )
                if "norm_joint_twice." in key:
                    tmp = copy.deepcopy(key)
                    state_dict[key.replace("norm_joint_twice.", "norm_joint_mid.")] = (
                        state_dict.pop(tmp)
                    )
