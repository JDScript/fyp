    ...
    def __init__(
        ...
        if self.is_input_continuous:
            ...
            self.proj_out_2 = nn.Linear(
                inner_dim, patch_size * patch_size * self.out_channels  # type: ignore
            )
    ...
    def forward(
        ...
        if self.is_input_continuous:
            ...
            output = hidden_states.reshape(
                shape=(
                    -1,
                    self.out_channels,  # type: ignore
                    height * self.patch_size,  # type: ignore
                    width * self.patch_size,  # type: ignore
                )
            )
...
class BasicMVTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        if self.cd_attention_mid:
            norm_hidden_states = (
                self.norm_joint_mid(hidden_states, timestep)
                if self.use_ada_layer_norm
                else self.norm_joint_mid(hidden_states)
            )
        ...
        if self.cd_attention_last:
            norm_hidden_states = (
                self.norm_joint_last(hidden_states, timestep)
                if self.use_ada_layer_norm
                else self.norm_joint_last(hidden_states)
            )
    ...
    def __call__(
        ...
        if multiview_attention:
            if not sparse_mv_attention:
                key = my_repeat(
                    rearrange(key_raw, "(b t) d c -> b (t d) c", t=num_views), num_views
                )
                value = my_repeat(
                    rearrange(value_raw, "(b t) d c -> b (t d) c", t=num_views),
                    num_views,
                )
            else:
                key_front = my_repeat(
                    rearrange(key_raw, "(b t) d c -> b t d c", t=num_views)[:, 0, :, :],
                    num_views,
                )  # [(b t), d, c]
                value_front = my_repeat(
                    rearrange(value_raw, "(b t) d c -> b t d c", t=num_views)[
                        :, 0, :, :
                    ],
                    num_views,
